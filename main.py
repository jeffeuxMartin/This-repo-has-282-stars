...
def get_stars():
    ...
    logger.info(
        "Successfully get stars: current stars:{}, current repo name:{}".format(
            r["watchers"], r["full_name"]
        )
    )
    return res
...
def update_project_name(data):
    ...
    if int(current_stars) != data["stars"]:
        ...
        new_data = {
            "name": new_name,
            "description": "這個倉庫有 {} 個 star ，不信你試試".format(data["stars"]),
        }
